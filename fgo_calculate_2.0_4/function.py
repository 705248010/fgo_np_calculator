...
2.对mooncell源代码进行解析
3.bug:对buff和伤害的小数点进行处理，让所有的数值计算都是浮点计算，最后输出为整数
注：5期泳装数据暂未加入（比较麻烦）,2100wDL强化已加入
'''
...
class MyWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.is_exist = True
        self.inherent_skill_info = []
        self.inherent_skill_buff = []
    ...
    def s_calculate(self):
        ...
        for key, value in self.effect.items():
            if ('Buster' or 'Quick' or 'Arts') in key and self.servant.servant_np_info[0][0] == re.findall(
                    '(Buster|Arts|Quick)+', key)[0]:
        ...
        # 获取职阶技能buff
        for buff in self.inherent_skill_buff:
            if re.findall('(Buster|Arts|Quick)+', buff) and self.servant.servant_np_info[0][0] in re.findall(
                    '(Buster|Arts|Quick)+', buff):
                if calculate_element['card_range'] != '0':
                    calculate_element['card_range'] = str(
                        eval(calculate_element['card_range'] + '+%s' % re.findall(r'\d+', buff)[0]))
                else:
                    calculate_element['card_range'] = re.findall(r'\d+', buff)[0]
            elif '宝具' in buff:
                if calculate_element['np_power'] != '0':
                    calculate_element['np_power'] = str(
                        eval(calculate_element['np_power'] + '+%s' % re.findall(r'\d+', buff)[0]))
                else:
                    calculate_element['np_power'] = re.findall(r'\d+', buff)[0]
            elif '伤害附加' in buff:
                if calculate_element['fixed_damage'] != '0':
                    calculate_element['fixed_damage'] = str(
                        eval(calculate_element['fixed_damage'] + '+%s' % re.findall(r'(?<=状态\()\d{1,3}', buff)[0]))
                else:
                    calculate_element['fixed_damage'] = re.findall(r'(?<=状态\()\d{1,3}', buff)[0]

        print(calculate_element)
    ...
    def set_servant_info(self):
        ...
        for file in dirs:
            if selected_servant == re.sub('[0-9]+', '', file):
                ...
                f_csv_4 = self.csv_reader(file + '/data_inherent_skill.csv')
                break
        if f_csv_1 == '' and f_csv_2 == '' and f_csv_3 == '' and f_csv_4 == '':
            self.is_exist = False
        else:
            ...
            for row in f_csv_4:
                if row[0] != '\ufeff序号':
                    self.inherent_skill_info.append(row[4])
            print(self.inherent_skill_info)
            self.is_exist = True
    ...
    def create_servant(self):
        self.servant = SelectServant(servant_np_info=self.servant_np_info, servant_atk_info=self.atk_info,
                                     ...
                                     inherent_skill_info=self.inherent_skill_info)
        self.atk, self.effect = self.servant.get_calculate_result()
        self.inherent_skill_buff = self.servant.inherent_skill()
        print(self.inherent_skill_buff)
