...
from inherent_skill import *


# 改一下宝具倍率的函数，改成传参（宝具等级，然后返回倍率）
class SelectServant:
    def __init__(self, servant_np_info, servant_atk_info, servant_level, rarity, _class, name, inherent_skill_info):
        ...
        self._class = _class
        self.name = name
        self.inherent_skill_info = inherent_skill_info
    ...
    def np_effect_of_pre(self):
        ...
        for i in range(len(self.servant_np_info)):
            for effect in effect_of_pre:
                if effect in self.servant_np_info[i][3] or '强大' in self.servant_np_info[i][3]:
                    if '强大' in self.servant_np_info[i][3]:
                        break
    ...
    # 计算职阶技能
    def inherent_skill(self):
        inherent_skill = []
        for effect in self.inherent_skill_info:
            for skill in inherent_skill_list:
                if skill in effect:
                    inherent_skill.append(effect)
        return inherent_skill
